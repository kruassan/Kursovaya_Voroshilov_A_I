    ...
    public partial class Admin_Predpriyatiya : Form
    {
        ...
        private void GridViewTable_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            if (((DataGridView)sender).SelectedCells[0].ColumnIndex == 0)
            {
                ...
                {
                    DataRow DR = W4A.GetTableFromZapros("select * from Предприятия where Код_предприятия = " + (((DataGridView)sender).SelectedCells[0].RowIndex + 1)).Rows[0];
                    new OnePredpritatiyeForm_Admin(DR[0].ToString(), DR[1].ToString(), DR[2].ToString(), DR[3].ToString(), DR[4].ToString(), W4A.GetTableFromZapros(/*"select * from ПерсДанныеРаботников where Код_работника = " + (((DataGridView)sender).SelectedCells[0].RowIndex + 1)*/"select ПерсДанныеРаботников.Код_работника, ПерсДанныеРаботников.Имя, ПерсДанныеРаботников.Фамилия, ПерсДанныеРаботников.Отчество, ПерсДанныеРаботников.Пол, ПерсДанныеРаботников.Дата_рождения from ПерсДанныеРаботников inner join Коды_рабочих_мест on ПерсДанныеРаботников.Код_работника = Коды_рабочих_мест.Код_работника where Коды_рабочих_мест.Код_предприятия=" + (((DataGridView)sender).SelectedCells[0].Value) + ";"), W4A).Show();
                }
            }
        }
