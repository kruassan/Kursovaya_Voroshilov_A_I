    ...
    public partial class Admin_Person_danniye : Form
    {
        ...
        private void GridViewTable_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            if (((DataGridView)sender).SelectedCells[0].ColumnIndex == 0)
            {
                ...
                {
                    DataRow DR = W4A.GetTableFromZapros("select * from ПерсДанныеРаботников where Код_работника = " + (((DataGridView)sender).SelectedCells[0].RowIndex + 1)).Rows[0];
                    new OneRabotnickForm_Admin(DR[0].ToString(), DR[1].ToString(), DR[2].ToString(), DR[3].ToString(), DR[4].ToString(), DR[5].ToString(), W4A.GetTableFromZapros("select * from Принятия_и_увольнения_работников where Код_работника = " + (((DataGridView)sender).SelectedCells[0].RowIndex + 1)), W4A.GetTableFromZapros("select * from Зачисления_зарплат where Код_работника = " + (((DataGridView)sender).SelectedCells[0].RowIndex + 1)), W4A).Show();
                }
            }
        }
